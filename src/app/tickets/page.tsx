...
function buildWhatsAppMessage(f: { name: string; phone: string; email: string; ticket: string; qty: number; total: number; mpesaRef: string }) {
  return (
    ...
    `*M-Pesa Ref:* ${f.mpesaRef.toUpperCase()}\n` +
    `━━━━━━━━━━━━━━━━━━━━━\n` +
...
export default function TicketsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = buildWhatsAppMessage({ name: form.name.trim(), phone: form.phone.trim(), email: form.email.trim(), ticket: ticket.name, qty, total, mpesaRef: form.mpesaRef.trim() })
  ...
  const whatsappMessage = buildWhatsAppMessage({ name: form.name.trim(), phone: form.phone.trim(), email: form.email.trim(), ticket: ticket.name, qty, total, mpesaRef: form.mpesaRef.trim() })

  const copyMessage = async () => {
    try { await navigator.clipboard.writeText(whatsappMessage); setCopied(true); setTimeout(() => setCopied(false), 2000) } catch { /* ignore */ }
  }
      ...
      <div className="min-h-screen bg-[#0f2419] flex items-center justify-center px-4 py-16">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-white/50 text-sm">WhatsApp opened — paste or send the message below to confirm your seat.</p>
          </div>
          ...
          <div className="bg-white/10 rounded-xl p-5 mb-5 space-y-2.5">
            {[
              ...
              { label: 'M-Pesa',   value: form.mpesaRef.toUpperCase(), mono: true },
            ].map((row) => (
          ...
          <div className="bg-[#1a4731] rounded-xl overflow-hidden mb-5">
            <div className="px-4 py-3 flex items-center justify-between">
              <span className="text-[#c9a84c] text-xs font-black uppercase tracking-wide">Message to send</span>
              <button onClick={copyMessage} className="text-xs text-white/70 hover:text-white px-3 py-1 rounded border border-white/20 hover:border-white/50 transition-all">
                {copied ? '✓ Copied!' : 'Copy'}
              </button>
            </div>
            <pre className="px-4 pb-4 text-xs text-white/80 whitespace-pre-wrap font-sans leading-relaxed">{whatsappMessage}</pre>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#0f2419]">
      ...
      <div className="max-w-4xl mx-auto px-4 pb-20">
        ...
        {/* ── Ticket tier cards (info only) ── */}
        <h2 className="text-white text-2xl font-black text-center mb-2">Ticket Options</h2>
        <p className="text-white/50 text-center text-sm mb-6">Choose your experience for the evening</p>
        ...
        <div className="rounded-2xl overflow-hidden shadow-2xl border border-white/10" style={{ backgroundColor: 'var(--bg-card)' }}>
          <div className="bg-[#1a4731] px-6 py-5">
            ...
            <p className="text-white/60 text-sm mt-1">Fill in your details after completing M-Pesa payment</p>
          </div>
          ...
          <div className="p-6 space-y-6">
                ...
                >
                  {TICKETS.map((t) => (
                    <option key={t.id} value={t.id}>
                      {t.name} — KSH {t.price.toLocaleString()}
                    </option>
                  ))}
                </select>
              </div>
            ...
            {/* Payment details */}
            <div className="rounded-xl border p-5" style={{ backgroundColor: 'var(--bg-alt)', borderColor: 'var(--border)' }}>
              <p className="text-sm font-semibold mb-4" style={{ color: 'var(--text-muted)' }}>
                Send <span className="text-[#c9a84c] font-black text-base">KSH {total.toLocaleString()}</span> via M-Pesa:
              </p>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm" style={{ color: 'var(--text-light)' }}>Business number</span>
                  <span className="text-2xl font-black" style={{ color: 'var(--text)' }}>400200</span>
                </div>
                <div className="border-t" style={{ borderColor: 'var(--border)' }} />
                <div className="flex items-center justify-between">
                  <span className="text-sm" style={{ color: 'var(--text-light)' }}>Account number</span>
                  <span className="text-base font-black tracking-wider" style={{ color: 'var(--text)' }}>01103084324001</span>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--text-muted)' }}>Phone Number *</label>
                  <input
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--text-muted)' }}>M-Pesa confirmation code *</label>
                <p className="text-xs mb-2" style={{ color: 'var(--text-light)' }}>From the SMS M-Pesa sent after payment, e.g. <strong style={{ color: 'var(--text)' }}>QFG4XXXXXW</strong></p>
                <input
                  type="text" name="mpesaRef" value={form.mpesaRef} onChange={handleChange}
                  placeholder="QFG4XXXXXW" maxLength={30}
                  className="w-full border rounded-lg px-4 py-3 text-sm font-mono uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-[#c9a84c]"
                  style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-alt)', color: 'var(--text)' }}
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 font-black text-sm rounded-xl transition-all disabled:opacity-40 flex items-center justify-center gap-2 hover:opacity-90"
              ...
              >
                {loading ? 'Sending…' : (
                  <>
                    ...
                    Confirm {ticket.name} × {qty} via WhatsApp — KSH {total.toLocaleString()}
                  </>
                )}
              </button>
            </form>
